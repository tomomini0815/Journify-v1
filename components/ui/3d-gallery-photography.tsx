...
function GalleryScene({
    ...
    // Load textures individually to handle errors gracefully
    useEffect(() => {
        const loader = new THREE.TextureLoader();
        const texturePromises = normalizedImages.map(img =>
    ...
    const materials = useMemo(
        () => Array.from({ length: Math.max(visibleCount, 1) }, () => createClothMaterial()),
        [visibleCount]
    );
    ...
    // Use state for plane data to ensure reactivity
    const [planesData, setPlanesData] = useState<PlaneData[]>([]);

    // Initialize/Update plane data when dependencies change
    useEffect(() => {
        if (totalImages === 0) return;

        setPlanesData(Array.from({ length: Math.max(visibleCount, 1) }, (_, i) => ({
            index: i,
    ...
    useFrame((state, delta) => {
        ...
        // Update plane data in state if it wraps
        const imageAdvance = totalImages > 0 ? visibleCount % totalImages || totalImages : 0;
        const totalRange = depthRange;

        // Note: For high-performance animation, we should ideally use refs for meshes.
        // But to stay close to the original logic during revert, we update state when wrap occurs.
        // For continuous Z movement, we'll let the next render handle it or use a separate ref for Z offsets.

        // Update positions in state (this will trigger re-renders, but matches original logic flow)
        setPlanesData(prevPlanes => {
            if (prevPlanes.length === 0) return prevPlanes;

            const nextPlanes = prevPlanes.map((plane, i) => {
                ...
                const normalizedPosition = ((newZ % totalRange) + totalRange) % totalRange / totalRange;

                // Update material uniforms directly for performance
                let opacity = 1;
                ...
                return {
                    ...plane,
                    z: ((newZ % totalRange) + totalRange) % totalRange,
                    imageIndex,
                    x: spatialPositions[i]?.x ?? 0,
                    y: spatialPositions[i]?.y ?? 0,
                };
    ...
    return (
        <>
            {planesData.map((plane, i) => {
                const texture = loadedTextures[plane.imageIndex];
                const material = materials[i];
                ...
                const aspect = textureImage && textureImage.width
...
function FallbackGallery({ images }: { images: ImageItem[] }) {
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full bg-gray-100 p-4 font-sans">
