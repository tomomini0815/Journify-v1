...
export function AdventureWorld() {
    ...
    return (
        <div className={`min-h-full transition-colors duration-1000 overflow-hidden relative rounded-3xl border border-white/10 ${isSolarpunk ? "bg-sky-50 text-slate-800 selection:bg-emerald-200" : "bg-[#050B14] text-white selection:bg-indigo-500/30"
            }`}>
            {/* Dynamic Background */}
            ...
            <div className={`absolute inset-0 z-0 pointer-events-none overflow-hidden transition-opacity duration-1000 ${isSolarpunk ? "opacity-50" : "opacity-0"}`}>
                ...
                <div className="absolute bottom-10 left-[40%] w-80 h-80 bg-white rounded-full blur-3xl opacity-30" />
            </div>

            {/* Content Container */}
            <div className="relative z-10 max-w-7xl mx-auto p-4 md:p-6 pb-20">
                {/* Header Navigation */}
                ...
                <AnimatePresence mode="wait">
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: 10 }}
                        className="mb-8 relative text-center md:text-left"
                    >
                        <h1 className={`text-4xl md:text-6xl font-black tracking-tight text-transparent bg-clip-text mb-2 filter drop-shadow-sm ${isSolarpunk
